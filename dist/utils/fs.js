...
export function getLanguage(root) {
    if (fileExists(join(root, "tsconfig.json")))
        return "typescript";
    if (fileExists(join(root, "requirements.txt")) || fileExists(join(root, "setup.py")))
        return "python";
    if (fileExists(join(root, "Cargo.toml")))
        return "rust";
    if (fileExists(join(root, "go.mod")))
        return "go";
    if (fileExists(join(root, "pom.xml")))
        return "java";
    if (fileExists(join(root, "Gemfile")))
        return "ruby";
...
export function getDescription(root) {
    const pkg = readJson(join(root, "package.json"));
    if (pkg?.description)
...
export function getLicense(root) {
    const pkg = readJson(join(root, "package.json"));
    if (pkg?.license)
        return pkg.license;
...
function _buildTree(dir, prefix) {
    let items;
    try {
...
export function getEnvVars(root) {
